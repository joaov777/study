...
#constantes
pressioneTecla="read -p \"Pressione qualquer tecla para continuar...\"  -n 1 s"
diretorioDeChaves="~/client-configs/files"
...
function procuraChaves() {
	...
	[ "$(ls -la $diretorioDeChaves/*$1* 2>1)" ] && {
		...
		ls -la ~/client-configs/files/*$1 && pressioneTecla
	} || {
		echo "|-- Usuário não tem chave criada" && pressioneTecla
	}
}
